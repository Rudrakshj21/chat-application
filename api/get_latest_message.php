...
try {
    $pdo = getPDO();
    $stmt = $pdo->prepare("SELECT  * FROM  message WHERE sender_id = :sid AND receiver_id = :rid  OR sender_id = :rid AND receiver_id = :sid ORDER BY id DESC LIMIT 1");
    $stmt->bindParam('sid', $sender_id);
    $stmt->bindParam('rid', $receiver_id);
    $stmt->execute();
    if ($stmt->rowCount() > 0) {
        response("fetched latest message from db successfully", true, $stmt->fetchAll(PDO::FETCH_ASSOC));
    } else {
